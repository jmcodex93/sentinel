...
class FakeTagWithTracks:
    def __init__(self, tracks=None, tag_type=0, name=""):
    ...
    def GetName(self):
        # A tag's NAME is the only piece of its identity that survives
        # save/reload (measured live, C4D 2026.303: GetGUID() doesn't
        # exist on a tag, and FindUniqueID(MAXON_CREATOR_ID) is
        # REGENERATED on load). Default "" so every pre-existing test in
        # this file keeps the un-named shape it was written with.
        return self._name
...
def test_capture_node_tracks_includes_tag_tracks(sentinel_module):
    ...
    # Owner format is "tag[<type>:<escaped name>:N]" (N7 fix): type and
    # NAME first, position among tags sharing BOTH last — the type-scoped
    # "tag[<type>:N]" of the N2 fix still mis-paired two same-type tags
    # that swap order. ``FakeTagWithTracks`` defaults to tag_type=0 and
    # name="", hence the empty middle field here.
...
def test_iter_node_tracks_owner_strings_unaffected_by_sentinel_pin_tag_count(sentinel_module):
    ...
    def owners_for(pin_count):
        ...
        # Same type AND same name on purpose: that is the one case where
        # the index is still the only discriminator (N7), so this test
        # keeps exercising the index dimension it was written for.
        tag_a = FakeTagWithTracks(tracks=[track_a], tag_type=1011, name="ctrl")
        tag_b = FakeTagWithTracks(tracks=[track_b], tag_type=1011, name="ctrl")
        pins_before = [
    ...
    assert baseline == {"a": "tag[1011:ctrl:0]", "b": "tag[1011:ctrl:1]"}
...
# --- N7: two tags of the SAME type must be told apart by NAME ------------
#
# The residual mis-pair the type-scoped ``tag[<type>:N]`` index left open
# (documented, not claimed closed, in the N2 section above): two tags of
# the same type on the same host — two Constraint tags, say — swapping
# order re-paired each pinned track onto the OTHER tag, applied it, and
# reported success. A mis-pair is invisible to ``plan_restore``: it looks
# exactly like a correct match.
#
# The key now carries the tag's NAME as well. Measured live (C4D
# 2026.303): a tag has no durable native id at all — ``GetGUID()`` is
# BaseObject API and doesn't exist on a tag, and
# ``FindUniqueID(MAXON_CREATOR_ID)`` is REGENERATED by save+reload — while
# ``GetName()`` survives. So type + name + position among homonyms is the
# most identity there is to key on.

def test_restore_pairs_by_tag_name_after_reordering_two_same_type_tags(sentinel_module):
    """N7: two Constraint-shaped tags (same type, DIFFERENT names) each
    animating the same parameter id, pinned, then SWAPPED in the host's
    tag list. Before the name entered the key, ``arriba``'s pinned keys
    were written onto ``abajo`` and vice versa — silently, reported as a
    clean success."""
    pin_tag = importlib.import_module("sentinel.ui.pin_tag")
    import c4d

    doc = FakeDoc()
    track_up = FakeTrack([(1000, 19, 5000)], c4d.CTRACK_CATEGORY_VALUE, keys=[
        FakeKey(c4d.BaseTime(0), 10.0),
    ])
    track_down = FakeTrack([(1000, 19, 5000)], c4d.CTRACK_CATEGORY_VALUE, keys=[
        FakeKey(c4d.BaseTime(0), 20.0),
    ])
    tag_up = FakeTagWithTracks(tracks=[track_up], tag_type=1011, name="arriba")
    tag_down = FakeTagWithTracks(tracks=[track_down], tag_type=1011, name="abajo")
    host = FakeTrackObject("ctrl", c4d, tags=[tag_up, tag_down], doc=doc)
    fake_tag = FakeTag(host, pin_tag.SENTINEL_PIN_TAG_PLUGIN_ID, c4d, doc)
    assert pin_tag._store_pin(fake_tag) is True

    # The artist wrecks both values, then reorders the two tags.
    track_up.GetCurve()._keys[0].value = -2.0
    track_down.GetCurve()._keys[0].value = -2.0
    host._tags = [tag_down, tag_up]

    report = pin_tag._restore(fake_tag)

    assert track_up.GetCurve()._keys[0].value == 10.0, (
        "'arriba' must get ITS OWN pinned value back after the reorder")
    assert track_down.GetCurve()._keys[0].value == 20.0, (
        "'abajo' must get ITS OWN pinned value back after the reorder")
    # And nothing went missing in the process — the reorder must be a
    # complete, honest re-pair, not a half-match dressed up as success.
    assert "no encontrada" not in report


def test_iter_node_tracks_indexes_same_named_tags_stably(sentinel_module):
    """N7 (b): two tags of the same type AND the same name are genuinely
    indistinguishable — they stay indexed 0, 1 in tag order, the same
    residual weakness ``location_keys`` accepts for same-named siblings.
    What must NOT happen is the index appearing only once a homonym shows
    up: indexing conditionally makes a lone tag's key rewrite itself the
    moment a second tag takes its name, silently invalidating every pin
    written before that (the exact bug ``location_keys`` was fixed for)."""
    pin_tag = importlib.import_module("sentinel.ui.pin_tag")
    import c4d

    def owners_for(tags):
        host = FakeTrackObject("ctrl", c4d, tags=tags)
        return [owner for owner, _track in pin_tag._iter_node_tracks(host)]

    def named(name):
        track = FakeTrack([(1000, 19, 5000)], c4d.CTRACK_CATEGORY_VALUE, keys=[
            FakeKey(c4d.BaseTime(0), 1.0),
        ])
        return FakeTagWithTracks(tracks=[track], tag_type=1011, name=name)

    lone = named("ctrl")
    second = named("ctrl")
    third = named("ctrl")

    # A LONE tag is already indexed — not bare.
    assert owners_for([lone]) == ["tag[1011:ctrl:0]"]
    # A second homonym does not rewrite the first one's key.
    assert owners_for([lone, second]) == ["tag[1011:ctrl:0]", "tag[1011:ctrl:1]"]
    # Nor does a third.
    assert owners_for([lone, second, third]) == [
        "tag[1011:ctrl:0]", "tag[1011:ctrl:1]", "tag[1011:ctrl:2]"]


def test_iter_node_tracks_indexes_per_name_not_per_type(sentinel_module):
    """N7 (c): the index counts among tags sharing the same (type, name)
    pair — NOT among all tags of the type. Two differently-named tags of
    one type are both ``:0``, which is what makes their keys invariant to
    each other's presence, order, and deletion."""
    pin_tag = importlib.import_module("sentinel.ui.pin_tag")
    import c4d
    ...
    host = FakeTrackObject("ctrl", c4d, tags=[named("arriba"), named("abajo")])
    owners = [owner for owner, _track in pin_tag._iter_node_tracks(host)]

    assert owners == ["tag[1011:arriba:0]", "tag[1011:abajo:0]"]


def test_restore_reports_a_renamed_tags_tracks_as_not_found(sentinel_module):
    """N7 (d): renaming a tag re-arms the pairing for its tracks — exactly
    like renaming an OBJECT re-arms its location key. That is a deliberate
    trade (a rename is the artist saying "this is something else now"),
    and the whole point of paying it is that the outcome is REPORTED on
    the row: the restore must not quietly claim it brought the animation
    back."""
    pin_tag = importlib.import_module("sentinel.ui.pin_tag")
    import c4d
    ...
    tag = FakeTagWithTracks(tracks=[track], tag_type=1011, name="arriba")
    host = FakeTrackObject("ctrl", c4d, tags=[tag], doc=doc)
    fake_tag = FakeTag(host, pin_tag.SENTINEL_PIN_TAG_PLUGIN_ID, c4d, doc)
    assert pin_tag._store_pin(fake_tag) is True

    track.GetCurve()._keys[0].value = -2.0
    tag.SetName("arriba_v2")

    report = pin_tag._restore(fake_tag)

    assert "1 pista no encontrada" in report, (
        "a renamed tag's pinned track must be reported as not found, "
