...
def test_format_crop_values_narrower_target_zooms_focal_and_pans_gate_relative():
    # 1:1 from 16:9 (1280x720 master): a narrower target zooms the FOCAL in by
    # source/target aspect (the universal crop lever, works on Redshift), and a
    # full nudge pans by the GATE-relative travel (source/target - 1)/2.
    src_focal = 36.0
    ...
    focal, fx, fy = framing.format_crop_values(src_focal, src_w, src_h, tw, th, nudge=(1.0, 0.0))
    assert focal == pytest.approx(src_focal * (sa / ta))     # 36 * 1.777 = 64.0
    assert focal == pytest.approx(64.0)
    ...
    f0, fx0, fy0 = framing.format_crop_values(src_focal, src_w, src_h, tw, th, nudge=(0.0, 0.0),
                                              source_film_x=0.02, source_film_y=-0.01)
    assert f0 == pytest.approx(64.0)
    assert fx0 == pytest.approx(0.02)
    assert fy0 == pytest.approx(-0.01)


def test_format_crop_values_wider_target_keeps_focal_and_pans_vertically():
    # 21:9 from 16:9: wider than source -> focal unchanged (resolution alone
    # crops top/bottom), and the nudge pans VERTICALLY.
    src_focal = 36.0
    ...
    focal, fx, fy = framing.format_crop_values(src_focal, src_w, src_h, tw, th, nudge=(1.0, 1.0))
    assert focal == pytest.approx(src_focal)                 # max(1, sa/ta)=1 -> unchanged
