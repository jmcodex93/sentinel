...
- **Review (2026-07-07, Codex impl + fixes MF-1/2/3, verificado live-MCP):** MF-1 corregido — el reader fijaba `is_checked=True` para el Main (false-RED en multiformato); ahora usa `take.IsChecked()`→`GetCurrentTake()` (verificado en vivo: Main excluido con children marcados). MF-2 — frames de AOV faltantes iban al bucket FAIL `missing`; ahora los shortfalls de streams AOV van solo a WARN `aov_missing` (§6.4, categoría única §7.2). MF-3 — `coverage_missing` solo si el stream beauty produjo 0 frames (no en gap parcial). **Hallazgos del primer run en PRODUCCIÓN real (SHOT_20, RS, beauty+AOVs en una carpeta) — RESUELTOS:** (1) **colisión de prefijo beauty↔AOV** (77 falsos size-outliers) — el prefijo beauty matcheaba los AOV hermanos que comparten base (`SHOT_..._010_AOV__Cryptomatte0168`), agravado por cortar el prefijo en el nº de shot `010`. **Fix A:** parser anclado `_frame_from_prefixed_stem` exige `<prefijo><1-sep?><dígitos-puros>` (rechaza AOVs); `scan_sequence` + `_paths_by_frame` lo comparten. **Fix B:** el reader doble-resuelve en `frame_from`/`+1` para detectar token de frame; sin token → prefijo = stem completo (no cortado en el shot). Validado en la carpeta real de 513 ficheros: 171 beauty, **0 size-outliers** (antes 77), 0 fugas de AOV. (2) **`detect_stale_cluster` cría-lobo con jitter sub-segundo** (0.5s → falso stale) — **Fix:** piso absoluto `MIN_STALE_GAP_SECONDS = 300` (una sesión previa está a minutos+); suite 0/30 flaky (antes ~12%).
**Carry-forwards abiertos (menores):** (a) **`_read_scene_render_state` con `except: return []`** → un fallo de lectura de escena se reporta como "nada que validar"/`passed=True` (false-GREEN) — considerar superficiar como ERROR. (b) `_render_history_target` clasifica file-vs-folder por `splitext`, así una carpeta de audit con punto en el nombre se malinterpreta como doc guardado.
...
- **Stale por pausa/jitter de render — RESUELTO con piso absoluto.** El modelo de corte-único (gap > `gap_factor`×mediana) disparaba falso `stale` no solo con pausas sino con **jitter sub-segundo de escritura** (verificado: 0.5s → `[0,1]` stale). Un solo run en producción lo confirmó (test flaky ~12%). Fix: `MIN_STALE_GAP_SECONDS = 300` — solo marca stale si el hueco es `> gap_factor×mediana` **Y** `≥ 300s` (una sesión previa está a minutos/horas). Los fixtures `stale_overwrite` (27h) y `long_render_spread` (sin hueco) siguen verdes; jitter/pausas breves ya no disparan. `stale` sigue siendo **WARN** (nunca FAIL).
