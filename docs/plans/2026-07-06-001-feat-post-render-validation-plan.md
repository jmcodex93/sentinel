...
- **Review (2026-07-07, Codex impl + fixes MF-1/2/3, verificado live-MCP):** MF-1 corregido — el reader fijaba `is_checked=True` para el Main (false-RED en multiformato); ahora usa `take.IsChecked()`→`GetCurrentTake()` (verificado en vivo: Main excluido con children marcados). MF-2 — frames de AOV faltantes iban al bucket FAIL `missing`; ahora los shortfalls de streams AOV van solo a WARN `aov_missing` (§6.4, categoría única §7.2). MF-3 — `coverage_missing` solo si el stream beauty produjo 0 frames (no en gap parcial). **Carry-forwards abiertos (verificar/afinar):** (a) **colisión de prefijo en AOVs de RS** — el `effective_path` Direct-Output sin dígitos de frame (`<base>_AOV_Diffuse`) hace que `_stem_prefix_for_scan` devuelva el stem sin separador final, así `Refractions` puede sombrear `Refractions Raw` de las tiers; contingente al residual U1 #1 (¿RS mete dígitos de frame en el effective_path de una secuencia?). (b) **`_read_scene_render_state` con `except: return []`** → un fallo de lectura de escena se reporta como "nada que validar"/`passed=True` (false-GREEN) — considerar superficiar como ERROR. (c) `_render_history_target` clasifica file-vs-folder por `splitext`, así una carpeta de audit con punto en el nombre se malinterpreta como doc guardado.
