...
- **Live en C4D**, que es donde esto se demuestra:
  ...
  - Doble clic en el tag desde el Object Manager restaura (o, si `MSG_EDIT` no llega a un tag, queda anotado como atajo no disponible y el botón `Restaurar` es el camino). **Medido en C4D 2026.303**: `MSG_EDIT` **sí llega** a un `TagData` — verificado interceptando el manejador de `Message`, que dispara `_restore` correctamente al recibir el id 21. Queda por confirmar con un doble clic real del usuario sobre el tag en el Object Manager que C4D efectivamente enruta ESE gesto hasta aquí; lo medido es que el manejador funciona si el mensaje llega, no que el gesto de usuario lo dispare.
