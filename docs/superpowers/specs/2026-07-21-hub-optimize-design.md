...
- **Gate de la toolbar deliberadamente grosero**: `shrinkEnabled`/`copyEnabled` en `HubPage` solo miran `status`/`asset_type` de la selección (¿hay al menos una fila `ok` texture/hdri? ¿al menos una `absolute`?), no la elegibilidad exacta contra un `target_px`. La elegibilidad fina (¿supera el objetivo elegido?) es responsabilidad de `shrinkPreview`, calculada dentro del diálogo una vez el artista elige 4K/2K/1K — documentado inline en `HubPage.tsx` y `HubToolbar.tsx`. El botón puede quedar habilitado con una selección que termine 100% en `skipped` (p. ej. todo ya ≤ objetivo); el diálogo lo comunica antes de confirmar, no es un estado de error.
- **Rounding-mode entre el preview TS y el servidor**: `shrinkPreview` (TS, `hubTable.ts`) usa `Math.round` para las nuevas dimensiones; `shrink_plan` (Python, `assets.py`) usa `round()`, que es round-half-to-even. Ambos coinciden en la inmensa mayoría de los tamaños reales; divergen solo en el caso exacto de un `.5` en el escalado (p. ej. una dimensión que cae justo en `x.5px` tras aplicar `target_px / max(w,h)`), donde JS redondea siempre hacia arriba y Python puede redondear hacia el par más cercano. Sin impacto práctico observado (diferencia de 1px en un lado, dentro del margen que ya introduce cualquier reescalado); no se corrigió porque forzar un modo de redondeo idéntico en TS complicaría el preview sin beneficio medible — anotado aquí para no perder el rastro si algún día el preview y el resultado real difieren en 1px.
