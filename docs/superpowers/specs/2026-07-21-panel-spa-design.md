...
- **Incógnita del host a verificar en 6.0** (única que queda): comportamiento del gadget HTML **dockeado** (los hosts actuales son ventanas async): foco, Cmd+Z passthrough, resize con el layout de C4D. Si el dock falla, fallback: el panel SPA vive como ventana flotante persistente y el nativo sigue para dock — decisión a tomar con el resultado del spike 6.0 en mano.
