...
- **Más núcleos sin-diálogo de los previstos**: la spec solo señalaba preset/reset-all como candidatos acoplados a `QuestionDialog`. La implementación extrajo 7 núcleos sin diálogo (`_apply_preset_core`, `_force_render_settings_core`, `_toggle_aspect_core`, `_add_sentinel_frame_tag_core`, `_toggle_light_groups_core`, `snapshot_save_still_core`, `snapshot_open_folder_core`) porque el mismo riesgo (un `MessageDialog`/Picture-Viewer bloqueando el drain de la cola del panel) aparecía en más motores de los anticipados (Sentinel Frame, Light Groups, snapshots). Mismo patrón de seguridad, mayor alcance — justificado, no una reescritura.
- **`light_groups` no es un tier de AOVs**: el op `aov_tier` acepta `light_groups` como tercer valor junto a `essentials`/`production`, pero internamente no aplica una lista de AOVs — llama a `_toggle_light_groups_core`, que activa/desactiva Light Groups en el canal Beauty. Se mantiene bajo el mismo op (y el mismo contrato de confirmación ⚠) por coherencia con la UI de 3 botones, pero la spec original lo agrupaba conceptualmente con `aov_tier` sin distinguir que es un toggle, no una lista.
- **Validate visible con Post-Render `unavailable`**: el botón "Validate →" del bloque Post-Render no se deshabilita cuando `postrender.available` es `false` (doc sin guardar o sin informe previo) — el deep-link a Reports funciona igual (la página Render Validation ya tiene su propio empty state). La spec no especificaba este caso; se decidió no bloquear el acceso a la página solo porque el resumen inline no tiene datos.
