...
def _find_object_match(entries, check_id, identity, result, stale_keys):
    ...
    for entry in entries:
        ...
        if same_location:
            ...
            # made. A differing guid says nothing (see the note by
            # _object_identity_fields): it differs after every reopen.
...
def _entry_key(entry):
    ...
    if kind == "object":
        ...
        # its LOCATION: two different objects cannot occupy the same path and
        # sibling index at once, so the guid adds nothing to identity — while
        # including it made a re-accept after reopening the scene write a
        # SECOND entry (the guid had been regenerated by the save), and left
        # the earlier entry permanently unaddressable, because the only key a
        # caller can build carries today's guid. The guid's real job —
