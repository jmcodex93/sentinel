...
- [x] **Fase 5 — Asset Hub en SPA** ✅ (v1.17.0): página `hub` con inventario virtualizado (tanstack/react-virtual), repathing con pending model + undo único, gate/collect inline como job en vivo (JobRegistry + `/thumb` binario); entradas migradas con fallback nativo
  ...
  - [x] **Fase 5.2 — Shrink + Copy into project** ✅ (v1.18.0, spec `2026-07-21-hub-optimize-design.md`): selección múltiple (single/toggle/shift-rango) en el Hub, Shrink por objetivo en K (4K/2K/1K) como job con progreso (slot único compartido con collect, copias hermanas `_2K` con original intacto, relink del lote en un undo), Copy into project síncrono a `tex/` con colisión segura (mismo tamaño → reusa, distinto → error de fila). pytest 630, vitest 43. Verificación live pendiente
  - [x] **Fase 5.3 — Conmutador de variantes de resolución** ✅ (v1.18.0, spec `2026-07-21-hub-variants-design.md`): detección pura (`split_res_token`/`find_res_variants`, sufijo del Shrink + tokens genéricos 1k-16k, último token manda), ops `hub/variants` + `hub/switch_res` (relink puro síncrono, forma de ruta conservada, un undo), SPA (indicador ⇄, diálogo "Switch res..." con contadores por objetivo). pytest 664, vitest 50. Verificación live pendiente (junto con Fase 5.2)
- [x] **Fase 6.0 — Host dockeable + shell + Overview** ✅ (v1.19.0, spec madre `2026-07-21-panel-spa-design.md`): `SentinelPanelSPA` registrado en paralelo al panel nativo (intacto), rail adaptativo (<560px iconos+badges / ≥560px sidebar), header + dashboard "salud del shot" de 4 tarjetas (QC/Assets/Render/Versión) con quick-fixes vía palette y deep-links a Hub/Reports. Motor `panel_ops.py` (`panel/state_stamp`, `panel/overview` con bloques aislados, `panel/open_form`) sobre los motores existentes, sin lógica duplicada. Spike PostWebMessage resuelto (push descartado, polling de stamp). pytest 682, vitest 60. **Verificación live pendiente** — incluye el spike de dock del webview (foco, Cmd+Z passthrough, resize de breakpoint, persistencia de layout)
- **Fase 6.1-6.4 (pendientes)**: resto de la migración del panel nativo a la SPA — candidatas a definir tras el resultado del spike de dock 6.0 (paridad de pestañas Render/Versions/Tools, retiro gradual del panel nativo si el dock funciona, o fallback a ventana flotante persistente si no)
