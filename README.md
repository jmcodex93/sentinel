...
- **Per-project rules** (`sentinel_rules.json`) — FPS, start frame, approved presets, default names, safe-area insets, the studio template scene (`template_scene`, the `.c4d` a project starts from — Reset All harvests its render presets; relative paths resolve against the folder of the rules file that declared them, and a declared path that isn't there makes Reset All refuse rather than silently use the plugin's bundled template) and per-check severity/on-off, publishable to a shared folder so a whole team validates against the same ruleset. Rules are discovered from the scene folder upward (nearest wins), override the per-machine settings, and take effect without restarting C4D.
